...
static void CVB_ReplyStatusUsage(int client)
{
	CReplyToCommand(client, "%t %t: sm_cvb_status <target|steamid|accountid>", "Tag", "Usage");
	CReplyToCommand(client, "%t %t", "Tag", "SupportedSteamIDFormats");
}
...
public Action Command_Status(int client, int args)
{
	...
	if (targetClient > 0 && IsValidClient(targetClient))
		CReplyToCommand(client, "%t %t", "Tag", "RestrictionStatusCheckingPlayer", targetClient);
	else
		CReplyToCommand(client, "%t %t", "Tag", "RestrictionStatusVerifying", targetDisplay);

	CVB_QueueFullRestrictionLookup(client, targetAccountId, targetClient, targetDisplay, GetCmdReplySource());
	return Plugin_Handled;
}
...
void Continue_BanIdentity_Async(AsyncContext context)
{
	...
	int banType = context.RestrictionMask;
...
void Continue_CheckIdentity_Async(AsyncContext context)
{
	...
	CReplyToCommand(admin, "%t %t", "Tag", "RestrictionStatusVerifying", context.TargetSteamId);
	CVBLog.Commands("Admin %N checking player identity %s (AccountID: %d)", admin, context.TargetSteamId, targetAccountId);

	CVB_QueueFullRestrictionLookup(admin, targetAccountId, FindClientByAccountID(targetAccountId), context.TargetSteamId, context.CommandReplySource);
}
