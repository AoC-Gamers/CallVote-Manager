...
def detect_default_workspace(root: Path) -> Path | None:
    if root.as_posix().startswith("/mnt/"):
        return Path("/tmp/callvote-manager-build")
    return None
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compile CallVote-Manager SourceMod plugins into a local build directory.")
